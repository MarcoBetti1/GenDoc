...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", help="Path to a gendoc.config.toml file with pipeline settings.")
    parser.add_argument("--repo", default=None, help="Path to the repository to document.")
    parser.add_argument("--out", default=None, help="Destination Markdown file.")
    parser.add_argument("--use-existing-docs", dest="use_existing_docs", action="store_true", help="Include existing documentation as supplemental context.")
    parser.add_argument("--no-existing-docs", dest="use_existing_docs", action="store_false", help="Skip existing documentation ingestion even if enabled in config.")
    parser.set_defaults(use_existing_docs=None)
    ...
    parser.set_defaults(enable_batching=None)
    ...
    parser.set_defaults(enable_reviewer=None)
    parser.add_argument("--llm-provider", choices=["mock", "openai"], default=None, help="LLM backend to use.")
    parser.add_argument("--llm-model", help="Override the default model for the configured provider.")
    parser.add_argument("--max-chunk-tokens", type=int, default=None, help="Maximum token budget per code chunk.")
