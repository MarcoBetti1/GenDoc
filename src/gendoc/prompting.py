...
class MockLLM:
    ...
    def complete(self, *, system_prompt: str, user_prompt: str, metadata: Optional[Dict[str, object]] = None) -> str:  # noqa: D401
        ...
        if "Repository snapshot:" in user_prompt:
            snapshot = self._extract_between(user_prompt, "Repository snapshot:", "Write a Markdown section") or ""
            return self._mock_repo_layout_section(snapshot)

        if "CLI source:" in user_prompt:
            cli_source = self._extract_between(user_prompt, "CLI source:", "Produce a Markdown section") or ""
            return self._mock_cli_reference(cli_source)
    ...
    def _mock_repo_layout_section(self, snapshot: str) -> str:
        ...
        for raw_line in snapshot.splitlines():
            ...
            if not raw_line.startswith(" "):
                current = stripped
                top_entries.append(stripped)
            elif current:
                child_name = stripped.lstrip("- ")
                children.setdefault(current, []).append(child_name)
        if not top_entries:
            lines.append("- Repository structure details will be published soon.")
            return "\n".join(lines)
        for entry in top_entries:
            desc = self._describe_repo_entry(entry)
            sample_children = children.get(entry, [])[:3]
            if sample_children:
                child_list = ", ".join(f"`{name}`" for name in sample_children)
                desc = f"{desc} (e.g., {child_list})"
            lines.append(f"- `{entry}`: {desc}")
        return "\n".join(lines)

    def _describe_repo_entry(self, entry: str) -> str:
        key = entry.rstrip("/").lower()
        mapping = {
            "src": "Python package source code for GenDoc.",
            "docs": "Documentation inputs and generated artefacts.",
            "docs/generated": "Sample documentation outputs generated by GenDoc.",
            "samples": "Sample projects packaged with the repository.",
            "pyproject.toml": "Project metadata, dependencies, and build configuration.",
            "readme.md": "High-level overview and onboarding guide.",
            "project-goal.md": "Narrative describing project intent.",
            "design-spec.md": "Design considerations for GenDoc improvements.",
            "first-version-plan.md": "Initial roadmap and milestone planning.",
        }
        return mapping.get(key, "Key project asset maintained in the repo.")

    def _mock_cli_reference(self, cli_source: str) -> str:
        ...
        for match in pattern.finditer(cli_source):
            ...
            flags = re.findall(r"['\"](--?[^'\",\s]+)['\"]", body)
            if not flags:
                continue
            help_match = re.search(r"help\s*=\s*(['\"])(?P<text>.+?)\1", body, re.DOTALL)
            help_text = help_match.group("text") if help_match else "Refer to the CLI help output for details."
            help_text = re.sub(r"\s+", " ", help_text).strip()
            entries.append((flags, help_text))
        lines = ["## CLI Reference"]
        if not entries:
            lines.append("- Run `gendoc --help` to inspect the available flags in this repository.")
            return "\n".join(lines)
        for flags, help_text in entries:
            flag_list = ", ".join(f"`{flag}`" for flag in flags)
            lines.append(f"- {flag_list}: {help_text}")
        lines.append("- Execute `gendoc --help` for additional usage examples and defaults.")
        return "\n".join(lines)
...
class PromptTemplates:
    ...
    system_repo_layout: str
    prompt_repo_layout: str
    system_cli_reference: str
    prompt_cli_reference: str
...
class PromptOrchestrator:
    ...
    def describe_repo_layout(
        self,
        *,
        goal: str,
        snapshot: str,
    ) -> str:
        prompt = self._templates.prompt_repo_layout.format(
            goal=goal,
            snapshot=snapshot,
        )
        response = self._client.complete(
    ...
    def describe_cli_reference(
        self,
        *,
        goal: str,
        cli_source: str,
    ) -> str:
        prompt = self._templates.prompt_cli_reference.format(
            goal=goal,
...
DEFAULT_TEMPLATES = PromptTemplates(
    ...
    system_repo_layout="You are a developer experience writer who explains repository structures succinctly for engineers.",
    prompt_repo_layout="""Project goal: {goal}\n\nRepository snapshot:\n{snapshot}\n\nWrite a Markdown section titled `## Repository Layout` that lists the most relevant directories and notable files with one-sentence descriptions. Only describe items present in the snapshot. Highlight documentation, source, and sample areas when available. Keep bullets concise, grounded in the snapshot, and avoid referencing repositories or assets that are not part of this project.""",
    system_cli_reference="You are a developer advocate clarifying CLI behaviour for practitioners.",
    prompt_cli_reference="""Project goal: {goal}\n\nCLI source:\n{cli_source}\n\nProduce a Markdown section titled `## CLI Reference` that explains every CLI argument defined in the parser. For each entry, list the flag names in backticks and describe what they control, including defaults or requirement notes. Close with guidance on using `--help` for more detail. Base every detail on the provided source and avoid fabricating additional flags or commands.""",
)
