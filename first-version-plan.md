...
- **LLM Usage**: Deterministic prompt templates; no retrieval or fine-tuning yet.
- **Output**: A single consolidated Markdown document plus structured intermediate artifacts (per-method notes, module summaries, orchestration manifest).
...
3. **Contextual Purpose Derivation**
   - For each grouping proposed earlier, ask “What is the purpose of this in the context of <parent grouping>?” using the collected low-level notes as evidence.
   - Allow stepping up or down the hierarchy when prompts reveal gaps (e.g., re-query a subgroup for clarity).
4. **Hierarchical Synthesis**
   ...
   - Maintain traceability from high-level statements back to the underlying notes to support revisions.
5. **Assembly & Export**
...
- How should we chunk code to balance context richness with token limits?
- Which metadata (file path, dependencies, docstrings) most improves LLM accuracy?
- What formatting keeps the generated Markdown readable while signaling which sections are AI-authored?
- How do we cache or deduplicate prompts to control latency and cost in iterative runs?
...
2. **Static Analysis Layer** (Day 1-2)
   ...
   - Produce chunk objects with source text + metadata.
3. **Prompt Engine** (Day 2-3)
      - Draft prompt templates for low-level extraction, contextual purpose probing, and synthesis passes.
      - Implement reusable wrapper for OpenAI chat completions (retry, rate-limit handling, cost tracking).
   4. **Generation Pipeline** (Day 3-4)
      - Orchestrate flow: structure heuristics → per-element notes → contextual roll-ups → global synopsis.
      - Implement the ability to revisit lower levels when higher-level prompts indicate missing detail.
      - Assemble Markdown using templates that expose the hierarchy.
   ...
   6. **Demo & Validation** (Day 5)
      ...
      - Capture learnings for next iteration (e.g., grouping heuristics, prompt tuning, backtracking triggers).
...
- Notes on prompt templates, grouping heuristics, and configuration, living in a `docs/prototype-notes.md` file.

## Immediate Next Actions
- Choose the sample project (language + repository).
- Define initial prompt templates for ultra low-level, contextual purpose, and synthesis passes.
...
- Outline heuristics for LLM-guided grouping and criteria for when to recurse back into lower levels.
